...
func newBullet(p Vector, r float64, move Vector, piercing bool) *Bullet {
	sprite := LaserSprite
...
func (b *Bullet) IsOffScreen() bool {
	margin := 100.0
	return b.position.X < -margin ||
		b.position.X > ScreenWidth+margin ||
		b.position.Y < -margin ||
		b.position.Y > ScreenHeight+margin
}
